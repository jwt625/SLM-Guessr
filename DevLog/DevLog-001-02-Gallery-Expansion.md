...
# DevLog 001-05: Level 4 - Special Beams

**Date**: 2026-01-24
**Status**: In Progress
**Objective**: Implement Level 4 with 20 special beam samples (vortex, axicon, Laguerre-Gaussian, compounds)

---

## Overview

Level 4 introduces special beams with unique physical properties: orbital angular momentum (vortex beams), non-diffracting propagation (Bessel beams), and structured mode families (Laguerre-Gaussian). Unlike L3's GS-optimized patterns, L4 returns to analytical phase structures with well-defined physics.

---

## Sample Plan (20 samples)

### Vortex Beams (6 samples)
1. **vortex_charge_sweep**: Topological charge l=1→5, 16 frames - donut radius increases
2. **vortex_charge_1_rotation**: Charge l=1, phase offset 0→2π, 24 frames - shows phase singularity
3. **vortex_charge_2**: Charge l=2 static - higher-order OAM
4. **vortex_charge_3**: Charge l=3 static - demonstrates spiral phase structure
5. **vortex_opposite_charges**: Two vortices l=+2 and l=-2 at different positions - OAM interference
6. **nested_vortex**: Nested structure (l=1 inside, l=2 outside), radius sweep

### Axicon/Bessel Beams (4 samples)
7. **axicon_slope_sweep**: Conical phase slope increases, 16 frames - Bessel ring spacing decreases
8. **bessel_order_0**: Zero-order Bessel beam - bright center + rings
9. **bessel_order_1**: First-order Bessel beam - dark center, bright ring
10. **axicon_truncated**: Axicon with aperture size sweep - transition to Bessel pattern

### Laguerre-Gaussian Beams (4 samples)
11. **lg_radial_sweep**: Radial index p=0→3, fixed l=0, 16 frames - radial ring count increases
12. **lg_azimuthal_sweep**: Azimuthal index l=0→4, fixed p=0, 16 frames - Gaussian to vortex
13. **lg_p1_l1**: LG(1,1) beam - radial + azimuthal structure
14. **lg_p2_l2**: LG(2,2) beam - higher-order mode

### Compound Beams (6 samples)
15. **vortex_plus_lens**: Vortex l=2 + quadratic lens, curvature sweep, 16 frames - focused vortex
16. **vortex_plus_grating**: Vortex l=1 + binary grating, period sweep, 16 frames - OAM in diffraction
17. **vortex_plus_tilt**: Vortex l=2 + linear tilt, angle sweep, 24 frames - off-axis vortex
18. **double_vortex_separation**: Two l=1 vortices, separation sweep, 16 frames - vortex interaction
19. **vortex_array_2x2**: 2×2 vortex array, alternating charges ±1 - complex OAM field
20. **axicon_plus_vortex**: Axicon + vortex, charge l=0→3, 16 frames - high-order Bessel beams

---

## Technical Implementation

### New Pattern Module (`slm_guessr/patterns_L4.py`)
- `create_vortex_phase()`: Spiral phase exp(i*l*θ)
- `create_axicon_phase()`: Conical phase α*r
- `create_laguerre_gaussian_phase()`: LG mode phase
- `create_laguerre_gaussian_target()`: LG mode intensity
- `create_bessel_target()`: Bessel function intensity pattern

### Generator Functions (`slm_guessr/generator.py`)
Add 20 generator functions. Most use analytical phase → FFT → intensity (like L1). Compound beams superimpose multiple phase patterns.

### Key Parameters
- Vortex charges: l = 1, 2, 3, 4, 5
- Axicon slopes: 0.01 to 0.1 (radians per pixel)
- LG indices: p = 0-3 (radial), l = 0-4 (azimuthal)
- Frame counts: 16 (standard), 24 (rotations)
...
- Vortex beams carry OAM proportional to topological charge l
- Phase singularity at beam center (undefined phase)
- Donut intensity with radius ∝ √l

### Non-Diffracting Beams
- Bessel beams maintain transverse profile during propagation
- Generated by axicon (conical lens)
- Ring spacing inversely proportional to cone angle

### Laguerre-Gaussian Modes
- Complete orthogonal basis for paraxial beams
- Radial index p: number of radial nodes
- Azimuthal index l: OAM and azimuthal phase structure
- LG(0,0) is standard Gaussian beam

### Compound Beams
- Superposition of phase elements creates new beam properties
- Vortex + lens: focused OAM beam
- Vortex + grating: OAM transferred to diffraction orders
- Axicon + vortex: high-order Bessel beams with OAM
...
All 20 L4 samples successfully generated and integrated.

### Files Created
- `slm_guessr/patterns_L4.py`: Vortex, axicon, LG beam pattern functions
- 20 generator functions added to `slm_guessr/generator.py`
- L4_SAMPLES list with all 20 sample configurations
- 40 GIF files in `static/assets/L4/` (20 samples × 2 GIFs each)

### Current Project Status
**Total Samples**: 92 (32 L1 + 20 L2 + 20 L3 + 20 L4)
**Total GIF Files**: 184
**Manifest**: Updated with all 92 samples

### Key Observations
- Vortex beams show clear spiral phase structure and donut intensity
- Higher topological charges produce larger donut radii
- Axicon patterns create Bessel-like ring structures in intensity
- Laguerre-Gaussian modes demonstrate both radial and azimuthal structure
- Compound beams (vortex + lens, vortex + grating) show rich interference patterns
- Phase patterns are analytical (not GS-optimized), returning to L1/L2 style
- All samples now have animations (no static single-frame samples)
- Extended animations (32 frames) for opposite charge vortices and truncated axicon

### Zoom Feature Implementation
Added intensity zoom capability to make small beam features more visible:
- New `intensity_zoom` parameter in SampleConfig (e.g., 5.0 for 5x zoom)
- Zoom applied during FFT computation via zero-padding technique:
  - Input field is zero-padded by zoom factor before FFT
  - Provides higher resolution sampling of Fourier plane
  - Central region extracted to maintain output size
  - Physically correct: shows finer detail in intensity distribution
- Zoom factor displayed as grey text label in top-right corner of intensity GIFs
- L4 samples use 5x zoom (3x for multi-spot patterns like vortex+grating)
- Improves visibility of donut structures, ring patterns, and beam details

### Animation Improvements
Enhanced all L4 samples with animations (no static samples):
- **Vortex Charge 2 & 3**: Added rotation animation (24 frames)
- **Bessel Order 0**: Added slope variation (16 frames)
- **Bessel Order 1**: Added rotation animation (24 frames)
- **LG(1,1) & LG(2,2)**: Added rotation animation (24 frames)
- **Vortex Array 2x2**: Added synchronized rotation (24 frames)
- **Opposite Charge Vortices**: Extended from 16 to 32 frames
- **Truncated Axicon**: Extended from 16 to 32 frames

### Next Steps
- Implement Level 5: Compound Patterns (15 samples)
- Implement Level 6: Practical Applications (15 samples)
- Implement Level 7: Complex Shapes (40 samples)
- Consider adding more special beam variants (Hermite-Gaussian, Ince-Gaussian)
